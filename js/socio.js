...
function activar(id) {
  ...
  bootbox.confirm("¿Desea cambiar el estado del socio a Activo?", function(
    result
  ) {
    if (result == true) {
      $.ajax({
        ...
        bootbox.alert("Se cambió el estado a Activo correctamente");
      });
    }
  });
}
...
function eliminacion(id) {
  ...
  bootbox.confirm("¿Desea dar de baja el socio?", function(result) {
    if (result == true) {
      $.ajax({
